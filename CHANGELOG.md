...
- Per-image 30-second soft download timeout: a timed-out image is marked failed (remote link + callout, note text still updates) and its late network result is permanently discarded — it can never create a vault file. Obsidian's network API cannot abort the request itself; the bound is on result handling.
- Version-metadata consistency: if the frontmatter version update fails after the note body write, the plugin re-reads the note and — only when it is still exactly the just-applied content — safely rolls it back to the pre-import snapshot and trashes attachments created in that attempt (reused files kept). If the note was already edited again, it is left untouched and a high-severity notice asks for a re-import; orphaned attachment paths are reported if trashing partially fails.
### Changed
